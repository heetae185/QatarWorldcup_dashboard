...
function HorizontalBar(props) {
  ...
  const GetData = async () => {
    ...
    for (var i = 0; i < MAX_ABILITY; i++) {
      var newAbility = {};
      newAbility.ability_name = Object.keys(dataset[props.player_number])[
        i + 38
      ];
      newAbility.ability_value =
        dataset[props.player_number][
          Object.keys(dataset[props.player_number])[i + 38]
        ];
...
function TopPlayerHexagon(props) {
  const svgRef = useRef(null);
  const [data, setData] = useState([
    { ability_name: "", ability_value1: "", ability_value2: "" }
  ]);
  ...
  const calcHexPos = function (hex, portion) {
    ...
    for (var i = 0; i < hex.length; i++) {
      newHex[i][0] =
        (hex[i][0] - moveXaxis) * portion + (1 - portion) * d + moveXaxis;
      newHex[i][1] =
        (hex[i][1] - moveYaxis) * portion + (1 - portion) * d + moveYaxis;
    }
    return newHex;
  };
  ...
  const GetData = async () => {
    ...
    for (var i = 0; i < MAX_ABILITY_COUNT + 1; i++) {
      ...
      newAbility.ability_value1 =
        dataset[props.player_number][newAbility.ability_name];
      newAbility.ability_value2 =
        dataset[props.player_number][newAbility.ability_name];
      newPlayer[i] = newAbility;
    }
    //                          고정값    hexagon의 비율                                     svg 이동 정보
    newPlayer[0].ability_value1 =
      d / 2 + (d / 2) * calcPortion(newPlayer[0].ability_value1) + moveXaxis;
    newPlayer[0].ability_value2 =
      d * calcPortion(newPlayer[0].ability_value2) + moveYaxis;

    newPlayer[1].ability_value1 =
      d + (d / 2) * calcPortion(newPlayer[1].ability_value1, true) + moveXaxis;
    newPlayer[1].ability_value2 =
      d * calcPortion(newPlayer[1].ability_value2) + moveYaxis;

    newPlayer[2].ability_value1 =
      d + d * calcPortion(newPlayer[2].ability_value1, true) + moveXaxis;
    newPlayer[2].ability_value2 = d + moveYaxis;

    newPlayer[3].ability_value1 =
      d + (d / 2) * calcPortion(newPlayer[3].ability_value1, true) + moveXaxis;
    newPlayer[3].ability_value2 =
      d + d * calcPortion(newPlayer[3].ability_value2, true) + moveYaxis;

    newPlayer[4].ability_value1 =
      d / 2 + (d / 2) * calcPortion(newPlayer[4].ability_value1) + moveXaxis;
    newPlayer[4].ability_value2 =
      d + d * calcPortion(newPlayer[4].ability_value2, true) + moveYaxis;

    newPlayer[5].ability_value1 =
      d * calcPortion(newPlayer[5].ability_value1) + moveXaxis;
  ...
  const frame = [
    [hex_x2 + moveXaxis, hex_y1 + moveYaxis], // hexagon 최외각 좌표들
  ...
  const frames = [
    calcHexPos(frame, 0.75),
    calcHexPos(frame, 0.5),
    calcHexPos(frame, 0.25)
  ];
  ...
  useEffect(() => {
    ...
    const hex_path = d3
      .line()
      .x((v) => v.ability_value1)
      .y((v) => v.ability_value2); // hexagon value
...
function TopPlayerAbility(props) {
  ...
  const GetData = async () => {
    ...
    newAbility.ability_value =
      dataset[props.player_number][newAbility.ability_name] / 1000000;
    ...
    newAbility.ability_value =
      dataset[props.player_number][newAbility.ability_name] / 10000;
    ...
    <div className="div_right">
      <div className="div_right_top">
        ...
        <div className="div_right_top_right">
          <HorizontalBar
            player_number={PLAYER_NUMBER + 0}
            ability_category="공격"
          />
          <HorizontalBar
            player_number={PLAYER_NUMBER + 1}
            ability_category="미드필더"
          />
          <HorizontalBar
